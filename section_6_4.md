...
傳統方案依賴委員會內部的 BFT 共識來確保安全性。為了防止惡意節點控制委員會（即佔據超過 $2/3$ 席位以偽造共識），必須擴大委員會規模 $C_{base}$。其單輪通訊複雜度為委員會內部廣播的開銷：
...
為了進行公平比較，我們必須計算基準方案在不同安全需求下所需的最小委員會規模。假設委員會選舉服從超幾何分佈 $H(N, M, C)$，其中 $M = \lfloor f \cdot N \rfloor$ 為惡意節點總數。系統失效（被攻陷）的定義為惡意節點在委員會中佔據 BFT 共識所需的絕對多數（即 $k > 2C/3$）。失效機率 $P_{fail}$ 為：

$$P_{fail}(C) = \sum_{k=\lfloor 2C/3 \rfloor + 1}^{C} \frac{\binom{M}{k}\binom{N-M}{C-k}}{\binom{N}{C}}$$
...
| :--- | :--- | :--- | :--- | :--- |
| **低安全性 (Low)** | $10^{-2}$ (1%) | 6 | 7 | **0.7x** |
| **中安全性 (Medium)** | $10^{-4}$ (0.01%) | 18 | 7 | **6.6x** |
| **高安全性 (High)** | $10^{-6}$ (0.0001%) | 27 | 7 | **14.9x** |
| **金融級安全 (Critical)** | $10^{-9}$ (1ppb) | 36 | 7 | **26.4x** |

> *註：BlockDFL 的複雜度以 $C^2$ 計算；本方案以 $C^2$ 計算（假設 $p \to 0$）。數據基於 $N=100, f=0.3$ 的超幾何分佈計算得出，失效閾值設為 $2C/3$。*

如表 6.2 所示，傳統方案在低安全需求下具有競爭力，但隨著安全要求的提高，其開銷增長顯著。若要求金融級安全（失效機率十億分之一），BlockDFL 需要 $C=36$。雖然基於 BFT 的 $2/3$ 閾值使得委員會不需要像 $1/2$ 閾值那樣龐大，但 $O(C^2)$ 的複雜度仍使得其開銷達到 $36^2 = 1296$ 次訊息。相比之下，本方案固定 $C=7$ ($Cost=49$)，在高安全性場景下展現出約 26 倍的通訊效率提升。

![通訊複雜度比較圖](analysis/complexity_comparison.png)
*圖 6.4：不同安全需求下的通訊複雜度比較。綠線（Proposed）保持恆定低成本，而紅線（Baseline）隨著安全等級提升（$\epsilon$ 減小）而呈現二次方增長。注意 X 軸越向右代表安全要求越高（$\epsilon$ 越小）。*
...
$$Cost_{base(Critical)} \approx O(36^2) = 1296$$
即便在 1% 的極端挑戰率下，本方案仍保有約 8.7 倍 ($1296/149$) 的效率優勢。
